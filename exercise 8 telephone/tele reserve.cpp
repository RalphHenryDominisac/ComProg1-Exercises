...
int main (){
	...
	double chargePremium;
	double chargePremiumTotal;
...
if (minutes >= 0 && minutes <= 75 && type == 'p' || type == 'P' ) {
       minutesDay = 25.00;
    cout << setprecision(2) << fixed << "$" << minutesDay << endl; }

if (minutes >= 75 && minutes <= 99 && type == 'p' || type == 'P' ) {
	dayMinutes = 75.00;
	b = minutes - dayMinutes;
	chargePremium = b * 0.10;
	chargePremiumTotal = chargePremium + 25;
	cout << setprecision(2) << fixed << "$" << chargePremiumTotal << endl; }

if (minutes2 >= 0 && minutes2 <= 100 && type == 'p' || type == 'P' ) {
    minutesNight = 25.00;
    cout << setprecision(2) << fixed << "$" << minutesNight << endl; }

if (minutes2 > 100  && type == 'p' || type == 'P' ) {
    nightMinutes = 100.00;
    c = minutes2 - nightMinutes;
	chargePremium = c * 0.05;
	chargePremiumTotal = chargePremium + 25;
	cout << setprecision(2) << fixed << "$" << chargePremiumTotal << endl; }
